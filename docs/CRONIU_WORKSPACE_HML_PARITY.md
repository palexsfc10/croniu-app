...
| 18 | Gate de consistência transversal de renovações (frontend-only, zero migration, zero endpoint novo) — auditadas todas as telas com badge/alerta/ação de renovação: Home, Central de Ciclos, Central de Renovações, Cliente 360°, lista de Clientes e Assistente. Achado: Central de Ciclos (`cycle-central.ts`, `findSuccessor`/`hasOpenRenewalRequest`) e a lista de Clientes (`client-list.ts`, `is_nearing_end && !upcomingCycle`) ainda derivavam a elegibilidade de renovação com heurísticas próprias — nenhuma conhecia `RenewalCase.status`, então um ciclo fechado via "Encerrar sem renovar" podia reaparecer como renovação pendente nessas duas telas (mais um terceiro caso não documentado antes: a lista de Clientes). Cliente 360° já tinha o dado certo buscado (`renewalCases`) mas ainda rodava a heurística antiga em paralelo só para o link "Preparar renovação" — um caso real de regra de estado duplicada entre componentes. Corrigido substituindo as três por um único helper compartilhado (`renewal-status.ts`: `buildRenewalCaseIndex`/`cycleRenewalCase`, um `Map<source_cycle_id, RenewalCaseView>`), alimentado em cada tela pelo mesmo `GET /renewal-cases?scope=all` já existente — nenhum endpoint novo, sem N+1 (uma query por carregamento de página, igual a antes). `cycle-central.ts` perde `findSuccessor`/`hasSuccessor`/`hasOpenRenewalRequest`; `client-list.ts` ganha `renewalCase` no `ClientRow` e usa o índice para decidir a razão `"renewal"`; `client-profile.tsx` unifica seu próprio caminho duplo num só. IA não precisou de nenhuma mudança: `_list_renewal_cases` já chama `renewal_case_svc.list_renewal_cases` diretamente, a mesma fonte que todas as telas agora compartilham — consistência garantida por construção, não por sincronização manual. Também esclarece uma inconsistência documental do relatório de chat da fatia 17 ("~18 organizações" era leitura errada de uma amostra truncada; o baseline real, confirmado por `select count(*) from organizations`, é 273) — corrigido na linha da fatia 17 acima, sem alterar dado nenhum | `cc17818` | `up-web` — api/admin preservados | **feito** — 88 testes de frontend focados (reescritos para o índice de `RenewalCase`: `cycle-central.test.ts`, `client-list.test.ts`, `cycles/page.test.tsx`, `clients/page.test.tsx`, `client-profile.test.tsx`, `renewals/page.test.tsx`) + typecheck/lint limpos; validado ao vivo em HML com conta sintética e 2 clientes cobrindo os 2 casos-chave: ciclo encerrado com `ended_without_renewal` (nunca aparece como pendente em nenhuma das 6 telas nem na resposta da IA) e ciclo `overdue` ainda precisando decisão (aparece com o rótulo idêntico "Renovação atrasada" em Home/Clientes/Ciclos/Cliente 360°/Renovações, e a IA relatou exatamente o mesmo caso ao ser perguntada "quais clientes precisam de decisão sobre renovação agora?"); conta sintética removida por ID exato (cascata confirmada: 0 clientes/renewal_cases residuais, contagem de organizações de volta a 273) |
| 19 | Design System Croniu — fundação + aplicação transversal parcial. Gate: 537 testes (não 528/529, 528/528 ou 533/533 dos relatórios anteriores — resolvido rodando a suíte completa 2× limpa após isolar 2 falhas diferentes sob carga como flakiness de contenção, não regressão real, confirmado reexecutando cada arquivo isolado). Auditoria (agente Explore) mapeou tokens/componentes/inconsistências reais. Fundação: tokens consolidados em `globals.css` (aliases nomeados para financeiro/renovações — sem matiz nova, só nome — `--gradient-ai`, `.icon-tile`, `outline-danger`, remoção de tokens/CSS mortos); 11 componentes oficiais novos (`IconButton`, `Select`, `SectionHeader`, `PageTitle`, `Avatar`, `MenuItem`, `Toast`, `TableShell`/`Th`/`Tr`/`Td`, `ListCard`, `AskAssistantLink`); página de referência `/app/dev/design-system` (gate client-side por hostname/`NODE_ENV`, nunca em PRD). Aplicação: as 7 tabelas independentes (Ciclos/Renovações/Financeiro/Serviços/Rotinas/Acompanhamentos/Cadastros pendentes) unificadas em `TableShell`; 32 páginas com H1 de página unificadas em `PageTitle` (ganho real: a maioria não tinha tamanho mobile próprio, ficava 30px fixo — agora 24px→30px em `lg:`); os 8 pontos "Perguntar à IA" (4 implementações visuais diferentes) unificados em `AskAssistantLink`; `ListCard` adotado em 5 das 13 telas com card mobile manual (Clientes/Ciclos/Financeiro/Renovações/Serviços) — redesenhado para não fixar `display`/padding (Tailwind não garante que uma classe posterior no `className` sobrescreva uma anterior por ordem de DOM, só por ordem de geração da folha de estilo — fixar `block` teria quebrado silenciosamente os cards `flex`); orbe do Assistente na bottom nav mobile (gradiente `--gradient-ai` + halo respeitando `prefers-reduced-motion`, verificado via regra CSS real, não suposição) substituindo o ícone genérico, sem tocar rota/thread/voz/proposta/confirmação do Assistente. Breakpoint bug real corrigido: `settings/layout.tsx` escondia a sidebar em 768px (`md:`) em vez de 1024px (`lg:`) como o resto do app. Decisão de escopo documentada (não uma limitação silenciosa): o pedido de painel lateral desktop persistente + camada full-screen mobile para o Assistente foi conscientemente reduzido ao orbe como ponto de entrada global — reconstruir a montagem do Assistente (1448 linhas, threads/voz/`ProposalCard`/confirmação obrigatória) como painel sobreposto à rota atual é uma mudança de arquitetura real, não uma unificação visual, e arriscaria exatamente os fluxos que o usuário pediu para preservar intactos; registrado para uma fatia dedicada, não implementado às pressas | `0ad831e`, `fdbb659`, `1d67ac0`, `2715c31`, `ba362f8` | `up-web` ×5 — api/admin preservados em todos | **feito, com telas restantes explicitamente pendentes** — 539/539 frontend a cada marco + typecheck/lint/build limpos; deploy e validação ao vivo a cada lote (contas sintéticas por lote, removidas por ID exato, 273 confirmado a cada limpeza) — reference page renderizada e navegável, títulos/avatar/tabelas conferidos com dados reais em Ciclos/Financeiro/Serviços, hrefs do `AskAssistantLink` conferidos em Home/Ciclos/Renovações, orbe conferido visualmente em mobile (375×812) e tablet (768×1024, chrome mobile correto, confirma o fix de breakpoint), 1920×1080 sem overflow, regra `prefers-reduced-motion` do orbe confirmada via inspeção da folha de estilo computada. Não migrado nesta fatia (documentado, não esquecido): Rotinas/Acompanhamentos/Onboarding e Portal/Autenticação/Entrada pública foram auditados e não apresentaram achados acionáveis novos além do já corrigido (tabelas, títulos); 8 das 13 telas com card mobile manual (Assistente, Rotinas-quadro, Agenda) ficaram fora do `ListCard` por terem estrutura interna mais bespoke; os pills "Perguntar à IA" restantes como CTA-link (slot de disponibilidade em Agenda) não viraram `Badge`/`Button` — exigiria dar suporte polimórfico a esses componentes, mudança maior que o escopo desta fatia; sweep mecânico de `<select>` nativo → `Select` (14 arquivos) não feito — selects nativos já usam os tokens corretos, ganho seria só o chevron customizado, risco desproporcional ao valor num arquivo de formulário sensível (`workspace/page.tsx`); painel lateral/full-screen do Assistente com orbe interativo (histórico/nova conversa/minimizar/fechar) — não implementado, ver decisão de escopo acima; overview visual completo, hardening final, `main` e PRD — intocados, como instruído |
| 20 | Assistente Croniu como camada persistente — refatoração arquitetural dedicada (não uma extensão da fatia 19). Auditoria prévia mapeou todo o comportamento da página `/app/assistant` (1448 linhas): threads/histórico, criação de conversa, envio, voz/transcrição, prefills via `?prompt=&context=&returnTo=`, `ProposalCard`, confirmação obrigatória com `X-Request-Id`/`confirmation_key` reais, pending actions, locks de idempotência (`sendLockRef`/`actionLockRef`), limites/entitlement (`ai_rate_limited`/`ai_daily_limit`/`ai_entitlement_denied`), navegação para registros, estados loading/error/empty. Extraído para `use-assistant-conversation.ts` (o núcleo único) + componentes de apresentação (`message-list`, `composer`, `threads-list`, `context-sidebar`, `assistant-header`) recompostos em `assistant-experience.tsx` (`layout: "page"\|"panel"\|"overlay"`) — zero duplicação das 1448 linhas, verificado rodando a suíte de 19 testes já existente da página sem alterar nenhuma asserção de comportamento (só 2 mudanças de texto deliberadamente pedidas: "Assistente Croniu" e a nova mensagem inicial curta). `AssistantLayerProvider` (montado em `app-shell.tsx`) monta uma segunda instância independente do mesmo núcleo — decisão de escopo documentada: uma única instância compartilhada entre página e painel exigiria acoplar a página completa a um contexto global que ela não precisa, arriscando a suíte de 19 testes que a valida standalone; a conversa em si nunca se perde de verdade de qualquer forma, pois é uma thread real no backend, acessível pelas duas superfícies via "Histórico". Desktop: painel lateral fixo 440px, `useAssistantPanelSpacing` dá padding-right real ao conteúdo (nada fica escondido atrás), foco/tab-trap, Esc minimiza. Mobile: overlay quase-tela-cheia, `document.body.style.overflow="hidden"` trava o scroll de fundo, botão voltar do navegador minimiza via `history.pushState`/`popstate` em vez de navegar. Orbe (bottom nav mobile), botão da sidebar desktop e pill do topbar mobile chamam `open()` em vez de `<Link>`; `/app/assistant` continua funcionando inalterada como página completa/deep link, `Ctrl+K` (`CommandPalette`) intocado. Corrige também um bug real descoberto durante a extração: o rule set `react-hooks/refs`+`react-hooks/immutability` do ESLint rejeitava refs aninhadas num objeto `refs: {...}` retornado pelo hook — corrigido achatando todos os refs como propriedades de primeiro nível do retorno do hook. Adicional: gate de `/app/dev/design-system` trocado de client-side (`useIsHmlOrDevEnvironment`, mantida como camada secundária inofensiva) para server-side real — `page.tsx` vira Server Component, lê o header `Host` via `next/headers` e chama `notFound()` de verdade fora de HML/dev (a rota passa a ser `ƒ` dinâmica no build, não estática) | `0ec4bdb` | `up-web` — api/admin preservados | **feito** — 546/546 frontend (5 testes novos do `AssistantLayerProvider`: abre/minimiza/restaura/fecha sem efeito colateral, suprimido em `/app/assistant`, prefill de contexto só em conversa vazia; 4 testes novos do gate server-side incluindo a chamada real a `notFound()`) + typecheck/lint/build limpos (build confirma `/app/dev/design-system` como rota dinâmica); validado ao vivo em HML com conta sintética: painel desktop abre sem cobrir a tela de Ciclos (conteúdo realmente reflui), consulta real respondida pela IA, mutação real (criar cliente) com `ProposalCard` completo confirmado, clique triplo rápido em "Confirmar" gerou exatamente 1 cliente no backend (idempotência real, não só visual — conferido via API), conversa sobreviveu à navegação SPA entre Ciclos→Clientes (Link real, não reload), minimizar/restaurar preserva rascunho não enviado, overlay mobile 390×844 com header completo/sugestões reais/composer fixo, scroll-lock e botão voltar confirmados via inspeção direta do DOM/history, `/app/dev/design-system` responde 200 em HML (host `croniu-hml.ntws.cloud`) confirmado via network log; cliente sintético e organização removidos por ID exato, 273 organizações confirmado. Limitação honesta: o painel e a página completa não compartilham o mesmo estado ao vivo (duas instâncias do mesmo núcleo) — ver decisão de escopo acima; validação de PRD real (404 genuíno) não executada ao vivo por não haver ambiente PRD acessível nesta fatia, coberta por teste unitário que verifica a chamada a `notFound()` |
