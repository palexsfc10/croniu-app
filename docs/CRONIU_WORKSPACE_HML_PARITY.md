...
| Assistente / IA | `/app/assistant` | Conversar, threads, tool calls | `agent.py` | Página Assistente workspace (histórico + contexto + atalhos + atividade) + palette global `Ctrl+K` + contexto `Perguntar sobre [cliente]`/`Perguntar à IA` no 360°/Agenda | Workspace 3 colunas | Camada operacional (resumo + acesso rápido + consultas recentes + destaque na nav) | Nenhuma ação sem confirmação explícita; usa só tools/dados oficiais | **feito** (fatia 6) — ações de ciclo (renovar/pausar/cancelar) seguem ausentes na IA, gap rastreado, não corrigido nesta rodada |
