...
| Detalhe do cliente — "Preparar acompanhamento" (E) | **Owner (print):** as 7 etapas do checklist têm peso visual idêntico (mesmo badge, mesmo botão cheio) — difícil identificar rapidamente o que precisa de ação | Observado no print anexado; `accompaniment/page.tsx` renderizava toda etapa (concluída ou não) com o mesmo cartão/botão | P2 | Alta — tela usada em todo cliente novo | Médio | Etapas concluídas colapsam (check + selo, sem botão); a etapa pendente seguinte real ganha destaque ("PRÓXIMO PASSO", borda de destaque, botão primário); barra de progresso no topo |
