# Entrega local — Admin premium e controle operacional
...
Nenhum commit, push ou deploy nesta entrega. A pasta originalmente aberta e suas alterações preexistentes foram preservadas.
...
[Abrir galeria local](./ADMIN_PREMIUM_PREVIEW.html).
...
- A autorização desta tarefa e a divergência das referências históricas de sprint estão documentadas em `AGENTS.md`, `PROJECT_STATE.md` e `SPRINT_ADMIN_CONTROL_PREMIUM.md` desta worktree.
- HML permanece na imagem anterior. Acesso ao Jarvis autorizado para auditoria não foi tratado como autorização de deploy desta interface.
...
O pedido de promoção para PRD foi recebido, mas não executado. O processo oficial (`docs/ops/PROMOTE_HML_TO_PRD.md`) exige `image_sha` imutável no GHCR, `deploy_sha` do pacote operacional, `build_run_id` e `deploy_bundle_run_id`, além de CI verde e rehearsal HML com os mesmos digests. A versão desta worktree contém alterações locais não commitadas e foi construída diretamente no Jarvis para HML com `admin-premium-20260904`; não há artefato GHCR correspondente.

PRD não foi alterado. Para continuar, é necessário versionar a mudança em um SHA aceito pelo repositório, executar os workflows oficiais de build/pacote, ensaiar os mesmos artefatos no HML e então promover pelo workflow protegido. O build direto com `:local` em PRD foi deliberadamente recusado pelo preflight e pelo runbook.
