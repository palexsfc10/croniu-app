...
test("network error can be retried; empty searches are honest", async ({ page }) => {
  ...
  await expect(page.getByText("Nenhuma organização encontrada", { exact: true })).toBeVisible();
});
...
test("directory history restores query, pagination and user organization links", async ({ page }) => {
  await mockPlatform(page);
  await page.goto("/organizations?search=Studio&page=2&size=50");
  await expect(page.getByRole("searchbox", { name: "Pesquisar registros" })).toHaveValue("Studio");
  await page.getByRole("button", { name: "Limpar busca" }).click();
  await expect(page).toHaveURL(/organizations\?size=50$/);
  await page.goBack();
  await expect(page).toHaveURL(/search=Studio&page=2&size=50/);
  await expect(page.getByRole("searchbox", { name: "Pesquisar registros" })).toHaveValue("Studio");
  await page.goto("/users?search=Marina&size=50");
  await expect(page.getByRole("link", { name: organization.name, exact: true })).toBeVisible();
  await page.getByRole("link", { name: organization.name, exact: true }).click();
  await expect(page.getByRole("heading", { name: organization.name })).toBeVisible();
});

test("logout network failure does not pretend the session was revoked", async ({ page }) => {
  await mockPlatform(page);
  let failed = true;
  await page.route("**/api/v1/platform/auth/logout", (route) => failed ? route.abort("failed") : route.fulfill({ json: { ok: true } }));
  await page.goto("/dashboard");
  await page.getByRole("button", { name: "Sair", exact: true }).click();
  await expect(page.getByRole("alert").filter({ hasText: "encerrar a sessão" })).toBeVisible();
...
test("directory loading and failed reads never display stale results", async ({ page }) => {
  await mockPlatform(page);
  let release!: () => void;
  const pending = new Promise<void>((resolve) => { release = resolve; });
  await page.route("**/api/v1/platform/organizations?**", async (route) => {
    await pending;
    await route.fulfill({ status: 503, json: { code: "unavailable", message: "Serviço indisponível" } });
  });
  await page.goto("/organizations");
  await expect(page.getByRole("status", { name: "Carregando organizações" })).toBeVisible();
  release();
  await expect(page.getByRole("main").getByRole("alert")).toContainText("Serviço indisponível");
  await expect(page.getByText("Nenhuma organização encontrada", { exact: true })).toHaveCount(0);
...
for (const width of [375, 768, 1280]) {
  test(`all console pages fit ${width}px and have no runtime errors`, async ({ page }) => {
    await mockPlatform(page);
    const errors: string[] = [];
    page.on("pageerror", (error) => errors.push(error.message));
    await page.setViewportSize({ width, height: 900 });
    for (const path of ["/dashboard", "/organizations", "/users", `/organizations/${organization.id}`, "/referrals", "/feedbacks", "/cycle-agenda", "/ai", "/errors"]) {
      await page.goto(path);
      await expect(page.getByRole("main").getByRole("heading", { level: 1 })).toBeVisible();
      await expect(page.getByRole("searchbox", { name: "Buscar organização" })).toBeVisible();
