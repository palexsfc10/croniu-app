...
def test_google_new_user_creates_account_org_and_trial(google_client, mock_verify, db_session):
    ...
    response = google_client.post("/api/v1/auth/google", json={"credential": "token-1"})
...
def test_google_returning_identity_logs_in_same_user_no_duplicate(
    ...
    claims = _claims()
    mock_verify(claims)
    first = google_client.post("/api/v1/auth/google", json={"credential": "token-a"})
    ...
    mock_verify(claims)
    second = google_client.post("/api/v1/auth/google", json={"credential": "token-b"})
...
def test_google_invalid_token_rejected(google_client, mock_verify):
    ...
    response = google_client.post("/api/v1/auth/google", json={"credential": "garbage"})
...
def test_google_token_missing_sub_rejected(google_client, monkeypatch):
    ...
    response = google_client.post("/api/v1/auth/google", json={"credential": "token-no-sub"})
    assert response.status_code == 401
    assert response.json()["code"] == "invalid_google_token"
...
def test_google_token_invalid_issuer_rejected(google_client, monkeypatch):
    ...
    monkeypatch.setattr(google_id_token, "verify_oauth2_token", fake_verify_oauth2_token)
    response = google_client.post("/api/v1/auth/google", json={"credential": "token-bad-iss"})
    assert response.status_code == 401
    assert response.json()["code"] == "invalid_google_token"
...
def test_google_token_expired_or_bad_signature_rejected(google_client, monkeypatch):
    ...
    monkeypatch.setattr(google_id_token, "verify_oauth2_token", fake_verify_oauth2_token)
    response = google_client.post("/api/v1/auth/google", json={"credential": "token-expired"})
    assert response.status_code == 401
    assert response.json()["code"] == "invalid_google_token"
...
def test_google_wrong_audience_rejected(google_client, monkeypatch):
    ...
    monkeypatch.setattr(google_id_token, "verify_oauth2_token", fake_verify_oauth2_token)
    response = google_client.post("/api/v1/auth/google", json={"credential": "token-wrong-aud"})
    assert response.status_code == 401
    assert response.json()["code"] == "invalid_google_token"
...
def test_google_email_verification_required_soft_gates_new_user(
    ...
    response = google_client.post("/api/v1/auth/google", json={"credential": "token-unverified"})
    assert response.status_code == 200
    body = response.json()
...
def test_google_existing_password_account_same_email_requires_link(
    ...
    mock_verify(claims)
    response = google_client.post("/api/v1/auth/google", json={"credential": "token-conflict"})
...
def test_google_link_with_correct_password_succeeds(
    ...
    response = google_client.post(
        ...
        json={"credential": "token-link", "password": register_payload["password"]},
    )
    assert response.status_code == 200
    assert "croniu_session" in response.cookies
    ...
    mock_verify(claims)
    second = google_client.post("/api/v1/auth/google", json={"credential": "token-relogin"})
    assert second.status_code == 200
...
def test_google_link_with_wrong_password_rejected(
    ...
    response = google_client.post(
        "/api/v1/auth/google/link",
        json={"credential": "token-link-bad-pw", "password": "senha-errada-123"},
    )
    assert response.status_code == 401
...
def test_google_login_blocked_for_disabled_organization(google_client, mock_verify, db_session):
    claims = _claims()
    mock_verify(claims)
    first = google_client.post("/api/v1/auth/google", json={"credential": "token-org-1"})
    assert first.status_code == 200
    ...
    mock_verify(claims)
    response = google_client.post("/api/v1/auth/google", json={"credential": "token-org-2"})
...
def test_google_only_account_cannot_password_login(google_client, mock_verify, db_session):
    claims = _claims()
    mock_verify(claims)
    google_client.post("/api/v1/auth/google", json={"credential": "token-pwless"})
    google_client.post("/api/v1/auth/logout")
...
def test_google_logout_revokes_session_created_via_google(google_client, mock_verify, db_session):
    claims = _claims()
    mock_verify(claims)
    login = google_client.post("/api/v1/auth/google", json={"credential": "token-logout"})
