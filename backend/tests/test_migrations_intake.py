...
def test_empty_database_upgrade_head_unique(migration_db: str):
    ...
    assert len(lines) == 1
    assert "0023_referral_coupon_program" in lines[0]

    current = _run_alembic("current")
    assert "0023_referral_coupon_program" in current.stdout
...
def test_upgrade_from_0018_to_head(migration_db: str):
    ...
    heads = _run_alembic("heads")
    assert "0023_referral_coupon_program" in heads.stdout
