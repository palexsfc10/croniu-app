...
def test_empty_database_upgrade_head_unique(migration_db: str):
    ...
    assert len(lines) == 1
    assert "0026_fixed_period_plan_pricing" in lines[0]

    current = _run_alembic("current")
    assert "0026_fixed_period_plan_pricing" in current.stdout
...
def test_upgrade_from_0018_to_head(migration_db: str):
    ...
    heads = _run_alembic("heads")
    assert "0026_fixed_period_plan_pricing" in heads.stdout
